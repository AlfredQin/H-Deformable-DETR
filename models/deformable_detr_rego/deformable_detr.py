    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.use_rego:
            ...
            for gi in range(len(self.rego_scales)):
                ...
                rego_outs = {'pred_logits_rego_%d' % gi: rego_output_classes[-1],
                             'pred_boxes_rego_%d' % gi: rego_output_coords[-1]}
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(
                self.get_loss(loss, outputs, targets, indices, num_boxes, extra_indices=extra_indices, **kwargs)
            )
...
def build(args):
    ...
    criterion = SetCriterion(
        num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha, use_rego=args.use_rego
    )
    criterion.to(device)
